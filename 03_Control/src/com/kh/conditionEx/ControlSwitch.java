...
public class ControlSwitch {
	...
	switch / case
	  ...
	  사용방법 2
	  	...
	  	case를 이어서 작성할 수 있음
	  		case1 : case 2 : case 3:
	  		 	system.out.print("케이스 123에 대한 결과");
	  		 	break;
	  		default:
	  			system.out.print("123 이외의 결과");
	  			
	  	break; 원하는 case를 만나면 case에 맞는 조건을 실행한 후
	  		   프로그램을 종료하길 원할 때 사용
	  		   case에 관련된 결과를 실행하기 전에 break를 호출하면 에러가 발생
	  		   
	   break와 return
	   break의 경우 switch나 for문 while문에서 사용
	   		    해당하는 case 이외를 종료함
	   return의 경우 어디서든지 사용 가능
	   			값을 반환할 수 있고, 메서드나 함수의 실행을 중단하고 호출한 곳으로 값을 전달
	   			메서드나 함수 내에서 return문이 실행되면 그 이후에 코드는 실행되지 않음
