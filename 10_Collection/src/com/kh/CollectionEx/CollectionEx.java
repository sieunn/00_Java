...
/*
 ...
 Collection은 List, set 의 부모클래스
 순차적으로 회전하면서 코드를 조회하거나 코드 안에 작성한 내용을 순회하고,
 추가 삭제 검색 등 기본 작업을 제공
 
 sort = 리스트를 기본 정렬 순서에 따라 정렬
 	1. 숫자 : 오름차순 정렬, 작은 값에서 큰값 순서로 정렬
 	2. 문자열 : 문자의 사전 순서에 따라 정렬
 	3. 우리가 파일을 만들어서 정한 기준 : compareTo() 메소드를 활용해서 정렬기준을 작성할수잇음
...
public class CollectionEx {
	public static void main(String[] args) {
		...
		//ArrayList 에서 add로 하나씩 추가하는 것이 아니라
		//한번에 값을 추가하기
									//()에 Arrays.asList()를 쓰면 넣고싶은만큼 추가가능
